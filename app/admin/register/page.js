...
export default function AdminRegister() {
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#111118", fontFamily: "Montserrat,sans-serif", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "2rem" }}>
      ...
      <p style={{ fontSize: "0.65rem", letterSpacing: "3px", color: "#1D9E75", marginBottom: "2.5rem" }}>CREATE ADMIN</p>
      ...
      <div style={{ background: "rgba(255,255,255,0.03)", border: "0.5px solid rgba(255,255,255,0.1)", borderRadius: "16px", padding: "2.5rem", width: "100%", maxWidth: "420px" }}>
        <h2 style={{ fontSize: "1rem", fontWeight: 400, letterSpacing: "2px", color: "#fff", marginBottom: "2rem", textAlign: "center" }}>Create Admin Account</h2>
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          <input className="inp" type="text" placeholder="Your Name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}/>
          <input className="inp" type="email" placeholder="Admin Email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })}/>
        ...
        <p style={{ textAlign: "center", marginTop: "1rem", fontSize: "0.72rem", color: "rgba(255,255,255,0.4)" }}>
          Already have account? <span onClick={() => router.push("/admin")} style={{ color: "#1D9E75", cursor: "pointer" }}>Login</span>
        </p>
      </div>
    </main>
  );
